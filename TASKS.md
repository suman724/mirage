...
| S3′ | **Seccomp supervisor (primary path)**: C launcher + Go notification loop (RECV/ID_VALID/ADDFD/SEND), path resolution from registers, reuse `Ensure()`, supervisor as PID 1 | ⬜ | Re-validate on Fargate with a libc tool AND a static Go binary; measure per-trap latency. Delete the C shim once green. Open: perf (every open traps), ADDFD fidelity, arm64, daemon reparenting. |
