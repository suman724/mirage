...
| 2.3 | Server local **cache** store (cache-store chaining: `cache → channelstore`); re-reads served locally | ✅ | `server/cache`: in-memory chunk.Store with single-flight (coalesces concurrent misses for the same hash). `Result` gains `CacheHits`. Manual e2e: 9 refs → 8 channel fetches + 1 cache hit (the duplicate file). Unit-tested (hit/miss, single-flight, no error caching). |
...
| 2.10 | **Reuse-over-reinvent review** of desync (and other deps): catalogue what they already provide and replace our hand-rolled equivalents where it fits | ⬜ | **Principle: don't reinvent the wheel.** Concrete leads already spotted in desync: `Cache` store (vs our `server/cache`), `DedupQueue` (vs our single-flight), `StoreRouter`/`Copy`/assemble helpers, FUSE helpers, `Index` binary format (vs our JSON manifest). Decide per item: adopt, adapt, or keep-with-reason. Key blocker to evaluate: desync's `Store` is `GetChunk(id) (*Chunk, error)` (no ctx) — adopting its Cache/DedupQueue likely means making `channelstore` implement `desync.Store` and threading ctx/timeout internally. Document each decision in CLAUDE.md. Should inform 2.3 (cache) and 2.5 (FUSE) — do this review **before** building more custom infra on top. |
